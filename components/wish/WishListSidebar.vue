      ...
      <transition name="fade" mode="out-in">
        ...
        <div v-else key="empty-wishlist" class="empty-wishlist">
          <div class="empty-wishlist__banner">
            ...
            <h1 class="empty-wishlist__label">{{ $t('Your wishlist is empty') }}</h1>
            <p class="empty-wishlist__description">
              {{ $t('') }}
            </p>
          </div>
          <SfButton data-cy="wishlist-sidebar-btn_start-shopping" class="sf-button--full-width color-secondary" @click="toggleWishlistSidebar">
            {{ $t('Go Back') }}
          </SfButton>
        </div>
      </transition>
